...
def eval_model(file: str, model, tokenizer, gpu_id: int = 6):
    ...
    with torch.autocast(f"cuda:{gpu_id}", dtype=torch.bfloat16):
        output_ids = model.generate(
            input_ids,
            images=[(image_tensor.unsqueeze(0).half().to(f'cuda:{gpu_id}'),
...
def convert_to_markdown(md_content, pdf_path):
    ...
    output_file = f'./got_output/{os.path.basename(pdf_path).replace(file_extension, ".mmd")}'
    os.makedirs("got_output", exist_ok=True)
    with open(output_file, "w", encoding="utf-8") as f:
        f.write(md_content)


def main(image_list: str,
         pdf_path: str,
         model,
         tokenizer,
         gpu_id: int = 6):
    ...
    for file_path in res_list:
        outputs += eval_model(file=file_path,
                              model=model,
                              tokenizer=tokenizer,
                              gpu_id=gpu_id)
...
def generate_mathpix_markdown(pdf_path: str, model, tokenizer, gpu_id: int = 6):
    ...
    outputs = main(image_list=image_list,
                   pdf_path=pdf_path,
                   gpu_id=gpu_id,
                   model=model,
                   tokenizer=tokenizer)
