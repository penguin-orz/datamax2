...
from datamax.utils import get_uno_manager, cleanup_uno_managers, HAS_UNO
...
def batch_convert_parallel(file_paths: list, max_workers: int = 4, use_uno: bool = True):
    """并行转换多个文档（使用UNO API）"""
...
def performance_comparison(file_paths: list):
    ...
    for key, name in methods:
        if key in results and results[key] is not None:
            ...
            logger.info(f"{name:<25} {total_time:<12.2f} {avg_time:<15.2f} {relative:<12}")
...
def convert_document_with_manager_info(file_path: str, use_uno: bool = True):
    ...
    try:
        ...
        result = parser.parse(str(file_path))
        elapsed_time = time.time() - start_time
        logger.info(f"✅ [线程{thread_id}] 转换成功: {os.path.basename(file_path)} (耗时: {elapsed_time:.2f}秒)")
        return result
        
    except Exception as e:
        elapsed_time = time.time() - start_time
        logger.error(f"❌ [线程{thread_id}] 转换失败: {os.path.basename(file_path)} - {str(e)} (耗时: {elapsed_time:.2f}秒)")
        raise


def batch_convert_with_manager_info(file_paths: list, max_workers: int = 4, use_uno: bool = True):
    """并行转换多个文档并显示管理器信息"""
    if not HAS_UNO and use_uno:
        logger.warning("⚠️ UNO API 不可用，将使用传统方式")
        use_uno = False
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
        # 提交所有任务
        future_to_file = {
            executor.submit(convert_document_with_manager_info, file_path, use_uno): file_path 
            for file_path in file_paths
        }
...
def traditional_stress_test(base_files: list, repeat_count: int = 3):
    ...
    # 性能对比表格
    logger.info(f"{'线程数':<8} {'总时间(秒)':<12} {'成功率':<10} {'吞吐量(文件/秒)':<18} {'并行效率':<12} {'效率百分比':<12}")
    logger.info("-" * 85)
    ...
    for max_workers in sorted(results.keys()):
        ...
        if 'error' in result:
            logger.info(f"{max_workers:<8} {'ERROR':<12} {'N/A':<10} {'N/A':<18} {'N/A':<12} {'N/A':<12}")
            continue
        ...
        efficiency_percentage = (efficiency / max_workers) * 100 if max_workers > 1 else 100
        
        if baseline_throughput is None:
            baseline_throughput = throughput
        
        logger.info(f"{max_workers:<8} {total_time:<12.2f} {success_rate:<10} "
    ...
    # 找出最佳配置
    valid_results = {k: v for k, v in results.items() if 'error' not in v and v['successful_count'] > 0}
    if valid_results:
        best_throughput = max(v['throughput'] for v in valid_results.values())
        best_config = [k for k, v in valid_results.items() if v['throughput'] == best_throughput][0]
        ...
        if len(valid_results) >= 3:
            ...
            logger.info(f"   📊 扩展性分析:")
            if efficiency_4 > 0:
                logger.info(f"      4线程效率: {efficiency_4:.2f}x ({(efficiency_4/4)*100:.1f}%)")
            if efficiency_8 > 0:
                logger.info(f"      8线程效率: {efficiency_8:.2f}x ({(efficiency_8/8)*100:.1f}%)")
            if efficiency_12 > 0:
                logger.info(f"      12线程效率: {efficiency_12:.2f}x ({(efficiency_12/12)*100:.1f}%)")
    ...
    logger.info(f"\n💡 传统方式优化建议:")
    if valid_results:
        best_efficiency_ratio = max((v.get('efficiency', 0) / k) for k, v in valid_results.items() if k > 1)
        ...
        logger.info(f"   📉 并行损失: {parallel_loss:.1f}% (理想{ideal_speedup}x vs 实际{actual_speedup:.2f}x)")
...
def comprehensive_stress_test(base_files: list, repeat_count: int = 3):
    ...
    if traditional_results and uno_results:
        ...
        logger.info(f"   传统方式: {traditional_threads}线程, {traditional_perf['throughput']:.2f}文件/秒")
    ...
    if traditional_results:
        best_traditional = max(v['throughput'] for v in traditional_results.values() if 'error' not in v)
        if HAS_UNO and uno_results:
            best_uno = max(v['throughput'] for v in uno_results.values() if 'error' not in v)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        # 默认进行传统方式压力测试
        logger.info("\n💡 使用参数指定测试模式:")
        logger.info("   python examples/uno_conversion_example.py traditional    # 仅测试传统方式")
        logger.info("   python examples/uno_conversion_example.py uno            # 仅测试UNO方式") 
        logger.info("   python examples/uno_conversion_example.py comprehensive  # 综合对比测试")
        logger.info("")
