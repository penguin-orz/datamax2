...
lock = threading.Lock()
retry_lock = threading.Lock()  # 用于控制重试的并发度
...
def llm_generator(
    ...
    """Generate content using LLM API with retry mechanism"""
    if not message:
        message = [
        ...
        try:
                ...
                if type == "question":
                    ...
                    if fmt_output is not None:
                        ...
                        if enable_retry and attempt < max_retries - 1:
                            # 多线程环境下的重试：添加随机延迟避免同时重试
                            with retry_lock:
                                thread_id = threading.current_thread().ident or 0
                                delay = (attempt + 1) * 0.5 + (thread_id % 100) * 0.01
                                time.sleep(delay)
                                print(f"⚠️ 网络状态不佳，正在重试... (第{attempt + 1}次重试)")
                                logger.warning(f"LLM JSON解析失败，第{attempt + 1}次重试")
                        else:
                            logger.error(f"LLM JSON解析失败，{'重试' + str(max_retries) + '次后仍无法解析' if enable_retry else '单次尝试失败'}")
                            return []
                else:
                    if output:
                        return [output]
                        ...
                        if enable_retry and attempt < max_retries - 1:
                            ...
                            with retry_lock:
                                ...
                                time.sleep(delay)
                                print(f"⚠️ 网络状态不佳，正在重试... (第{attempt + 1}次重试)")
                                logger.warning(f"LLM输出为空，第{attempt + 1}次重试")
                        else:
                            logger.error(f"LLM输出为空，{'重试' + str(max_retries) + '次后仍无输出' if enable_retry else '单次尝试失败'}")
                            return []
            else:
                # 响应格式错误，需要重试
                if enable_retry and attempt < max_retries - 1:
                    ...
                    with retry_lock:
                        ...
                        time.sleep(delay)
                        print(f"⚠️ 网络状态不佳，正在重试... (第{attempt + 1}次重试)")
                        logger.warning(f"LLM响应格式错误，第{attempt + 1}次重试")
                else:
                    logger.error(f"LLM响应格式错误，{'重试' + str(max_retries) + '次后仍错误' if enable_retry else '单次尝试失败'}")
                    return []
            ...
            if enable_retry and attempt < max_retries - 1:
                ...
                with retry_lock:
                    ...
                    time.sleep(delay)
                    print(f"⚠️ 网络状态不佳，正在重试... (第{attempt + 1}次重试)")
                    logger.warning(f"LLM网络错误: {e}，第{attempt + 1}次重试")
            else:
                logger.error(f"LLM网络错误，{'重试' + str(max_retries) + '次后仍失败' if enable_retry else '单次尝试失败'}: {e}")
...
def process_match_tags(
    ...
    def match_one_question(q):
        ...
        match = llm_generator(
            ...
            prompt=prompt,
            type="question",
            enable_retry=enable_retry,  # 支持多线程环境下的重试
        )
                ...
                if output:
                    ...
                    if json_output is not None:
                        ...
                        if attempt < max_retries - 1:
                            logger.warning(f"领域树生成失败，JSON格式错误，第{attempt + 1}次重试")
                        else:
                            print(f"❌ 领域树生成失败，重试{max_retries}次后仍无法生成有效JSON格式，将使用空树继续执行")
                            logger.error(f"领域树生成失败，重试{max_retries}次后仍无法生成有效JSON格式")
                            return DomainTree([])
                else:
                    # 输出为空，需要重试
                    if attempt < max_retries - 1:
                        logger.warning(f"领域树生成失败，输出为空，第{attempt + 1}次重试")
                    else:
                        print(f"❌ 领域树生成失败，重试{max_retries}次后仍无输出，将使用空树继续执行")
                        logger.error(f"领域树生成失败，重试{max_retries}次后仍无输出")
                        return DomainTree([])
            else:
                # 响应格式错误，需要重试
                if attempt < max_retries - 1:
                    logger.warning(f"领域树生成失败，响应格式错误，第{attempt + 1}次重试")
                else:
                    print(f"❌ 领域树生成失败，重试{max_retries}次后响应格式仍错误，将使用空树继续执行")
                    logger.error(f"领域树生成失败，重试{max_retries}次后响应格式仍错误")
                    return DomainTree([])
                    
        except Exception as e:
            if attempt < max_retries - 1:
                logger.warning(f"领域树生成失败，网络错误: {e}，第{attempt + 1}次重试")
            else:
                print(f"❌ 领域树生成失败，重试{max_retries}次后网络错误: {e}，将使用空树继续执行")
                logger.error(f"领域树生成失败，重试{max_retries}次后网络错误: {e}")
                return DomainTree([])
...
def find_tagpath_by_label(domain_tree: Optional[DomainTree], label: str):
    return domain_tree.find_path(label) if domain_tree else ""
...
def generatr_qa_pairs(
    ...
    message: Optional[list] = None,
    max_workers: int = 5,
    domain_tree: Optional[DomainTree] = None,
    enable_retry: bool = True,
) -> list:
    qa_pairs = process_answers(
        ...
        base_url=base_url,
        model=model_name,
        enable_retry=enable_retry,  # 传递重试参数
    )
    logger.success(
...
if __name__ == "__main__":
    ...
    domain_tree, domain_tree_success = process_domain_tree(
        api_key=API_KEY,
        base_url=BASE_URL if BASE_URL else "",
        model="qwen-plus",
        text="\n".join(page_content) if isinstance(page_content, list) else str(page_content),
        temperature=0.7,
        top_p=0.9,
        max_retries=3,
    )
    ...
    # check if domain tree generation failed
    if not domain_tree_success:
        print("⚠️ 当前大模型生成领域树失败！将使用纯文本模式生成问答对！请稍后或更换大模型重试！")
        logger.warning("领域树生成失败，切换到纯文本模式")
        # 为所有问题设置默认标签
        for question_item in question_info:
            question_item["label"] = ""
    else:
    ...
    r = generatr_qa_pairs(
        ...
        question_number=5,  
        max_workers=10,  
        domain_tree=domain_tree if domain_tree_success else None,
        # message=[] 
    )
